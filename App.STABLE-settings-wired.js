...
function SettingsScreen({ navigation }) {
  ...
  const reminderNotificationsStatus = 'Reminder notifications on';
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const [loadedFamilyMembers, loadedPosts, loadedMemories] = await Promise.all([
      ...
      if (Array.isArray(loadedFamilyMembers)) {
        setFamilyCounts({
          total: loadedFamilyMembers.length,
          pending: loadedFamilyMembers.filter((member) => member.status === 'pending').length,
        });
      }

      setCommunityPostCount(Array.isArray(loadedPosts) ? loadedPosts.length : 0);
      setMemoryCount(Array.isArray(loadedMemories) ? loadedMemories.length : 0);
  ...
  const menuSections = [
    {
      title: 'Account',
      items: [
        { icon: '👤', label: 'Profile', sub: 'Raymond · rayray579@gmail.com', onPress: () => Alert.alert('Profile', 'Profile editing is coming soon.') },
        {
          icon: '🏠',
    ...
    },
    {
      title: 'Notifications',
      items: [
        { icon: '🔔', label: 'Reminder Notifications', sub: reminderNotificationsStatus },
        { icon: '📣', label: 'Push Notifications', sub: pushNotificationStatus },
      ],
    },
    {
      title: 'Data & Storage',
      items: [
        { icon: '🐾', label: 'Pets', sub: countsLoading ? 'Loading...' : `${safePets.length}` },
        { icon: '📋', label: 'Health Records', sub: countsLoading ? 'Loading...' : `${healthRecordCount}` },
        { icon: '⏰', label: 'Care Reminders', sub: countsLoading ? 'Loading...' : `${reminderCount}` },
        { icon: '🖼️', label: 'Memories', sub: countsLoading ? 'Loading...' : `${memoryCount ?? 0}` },
        { icon: '💬', label: 'Community Posts', sub: countsLoading ? 'Loading...' : `${communityPostCount ?? 0}` },
    ...
    },
    {
      title: 'App Info',
      items: [
        { icon: '🧩', label: 'App Name', sub: 'PetSync+' },
        { icon: '🔢', label: 'Version', sub: 'v1.0.0' },
        { icon: '☁️', label: 'Supabase', sub: supabaseConnectedStatus },
      ],
    },
    {
      title: 'Support',
