...
function SettingsScreen({ navigation, route }) {
  ...
  const currentPlanLabel = isPremiumUser(authProfile) ? 'Premium' : 'Free';
  const revenueCatStatusLabel = revenueCatReady ? 'Ready' : 'Not connected';
  const revenueCatEntitlementLabel = revenueCatCustomerInfo?.entitlements?.active?.Premium ? 'Active' : 'Inactive';
  const displayName = authProfile?.display_name
    || authUser?.user_metadata?.display_name
    || authUser?.email?.split('@')?.[0]
    || 'Pet Parent';
  ...
  const menuSections = [
    ...
    },
    {
      title: 'Subscription',
      items: [
        { icon: '⭐', label: 'PetSync+ Premium', sub: canUseFeature(authProfile, 'ai_vet_assistant') ? 'Active' : 'Coming soon', onPress: handleSubscriptionPress },
      ],
    },
    {
      title: 'App Info',
            ...
            <View style={s.modalOverlay}>
              <View style={[s.customActionModal, s.subscriptionModal]}>
                ...
                >
                  ...
                  <Text style={s.featureLockedReason}>
                    Premium unlocks the full PetSync+ experience with unlimited pets, health tools, AI Vet, Family Sharing, and more.
                  </Text>
                  ...
                  <View style={s.subscriptionPlanCard}>
                    ...
                    <Text style={s.subscriptionPlanNote}>You can continue using the free plan while Premium is in progress.</Text>
                  </View>

                  <View style={s.subscriptionPlanCard}>
                    <Text style={s.subscriptionPlanLabel}>RevenueCat</Text>
                    <Text style={s.subscriptionPlanValue}>{revenueCatStatusLabel}</Text>
                    <Text style={s.subscriptionPlanNote}>Entitlement Premium: {revenueCatEntitlementLabel}</Text>
                  </View>

                  <View style={s.subscriptionPlanCard}>
                    <Text style={s.subscriptionPlanLabel}>Monthly</Text>
                    <Text style={s.subscriptionPlanValue}>PetSync+ Premium Monthly</Text>
                    <Text style={s.subscriptionPlanNote}>
                      {monthlyPackage ? `Package ID: ${monthlyPackage.identifier}` : 'Package not loaded yet.'}
                    </Text>
                    <Text style={s.subscriptionPlanNote}>Unlock the full experience on a month-to-month plan.</Text>
                  </View>

                  <View style={s.subscriptionPlanCard}>
                    <Text style={s.subscriptionPlanLabel}>Yearly</Text>
                    <Text style={s.subscriptionPlanValue}>PetSync+ Premium Yearly</Text>
                    <Text style={s.subscriptionPlanNote}>
                      {yearlyPackage ? `Package ID: ${yearlyPackage.identifier}` : 'Package not loaded yet.'}
                    </Text>
                    <Text style={s.subscriptionPlanNote}>Save with an annual plan when Premium launches.</Text>
                  </View>
                  ...
                  <Text style={s.subscriptionComingSoonNote}>Premium coming soon</Text>
                </ScrollView>
                <View style={s.customActionModalButtons}>
                  <TouchableOpacity
                    style={[
...
export default function App() {
  ...
  useEffect(() => {
    ...
    }, [authReady, authUser?.id, applyRevenueCatState]);
