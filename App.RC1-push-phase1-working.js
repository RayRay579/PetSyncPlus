...
const upsertAuthProfileToSupabase = async (user, displayName) => {
  if (!user?.id) {
    return null;
  }

  const nextProfileName = String(displayName || user.user_metadata?.display_name || user.user_metadata?.full_name || user.email?.split('@')?.[0] || 'Pet Parent').trim();
  const payload = {
    id: user.id,
    email: user.email || '',
...
const saveCommunityProfileToSupabase = async (profileKey, draft) => {
  try {
    ...
    const payload = {
      display_name: draft.displayName || row.display_name || row.full_name || row.name || '',
      full_name: draft.displayName || row.full_name || row.display_name || row.name || '',
      name: draft.displayName || row.name || row.display_name || row.full_name || '',
      avatar_emoji: draft.avatarEmoji || row.avatar_emoji || '🐾',
      avatar_url: draft.avatarUrl || row.avatar_url || '',
      bio: draft.bio ?? row.bio ?? row.about ?? '',
    };
...
function SettingsScreen({ navigation }) {
  ...
  const { careReminders } = useContext(CareRemindersContext);
  const { openAddPetModal } = useContext(AddPetContext);
  const { authUser, authProfile, signOut } = useContext(AuthContext);
  ...
  useEffect(() => {
    setProfileDraft((prev) => ({
      ...
      avatarEmoji: authProfile?.avatar_emoji || prev.avatarEmoji || '👤',
      avatarUrl: authProfile?.avatar_url || prev.avatarUrl || '',
      bio: authProfile?.bio || prev.bio || '',
    }));
  }, [authProfile, authUser, resolvedProfileEmail, resolvedProfileName]);
  ...
  const handleSaveProfile = async () => {
    const nextProfile = {
      ...profileDraft,
      displayName: String(profileDraft.displayName || '').trim() || 'Raymond',
      email: resolvedProfileEmail,
    };
    ...
    const saved = await saveCommunityProfileToSupabase(authUser?.id || 'raymond', nextProfile);
    if (!saved) {
          ...
          <TouchableOpacity activeOpacity={0.9} onPress={handleProfilePress}>
            <Card style={s.profileCard}>
              <View style={s.profileAvatarCircle}>
                <Text style={{ fontSize: 32 }}>{profileDraft.avatarEmoji || '👤'}</Text>
              </View>
              <View style={s.flex}>
                <Text style={s.profileName}>{resolvedProfileName}</Text>
                <Text style={s.profileEmail}>{resolvedProfileEmail}</Text>
                <Text style={s.menuSub}>Tap to edit your profile.</Text>
              </View>
        ...
        <Modal visible={showAccountModal} transparent animationType="fade" onRequestClose={() => setShowAccountModal(false)}>
          <View style={s.modalOverlay}>
            <View style={s.customActionModal}>
              ...
              <View style={[s.profileAvatarCircle, { marginBottom: 12 }]}>
                <Text style={{ fontSize: 32 }}>{profileDraft.avatarEmoji || '👤'}</Text>
              </View>
              <Text style={{ color: C.muted, marginBottom: 12, textAlign: 'center' }}>
                You can update your display name now. Email stays read-only for the moment.
              </Text>
              <TextInput
                style={s.customActionInput}
