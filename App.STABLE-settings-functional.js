...
function MemoryVaultScreen({ navigation }) {
  ...
  const saveMemory = () => {
    ...
    const nextMemory = {
      ...
      createdAt: new Date().toISOString(),
      syncStatus: 'pending',
      emoji: memoryDraft.photoUri ? null : getMemoryDisplayEmoji({ type: memoryDraft.memoryType, mimeType: memoryDraft.mimeType }),
    };
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 10 }}>
              ...
              <TouchableOpacity style={[s.petProfileButton, { marginBottom: 10 }]} onPress={pickMemoryMedia}>
                <Text style={s.petProfileButtonText}>
                  {memoryDraft.photoUri ? 'Change Media' : 'Pick Media'}
                </Text>
              </TouchableOpacity>
...
function SettingsScreen({ navigation }) {
  ...
  const renderSettingsItem = (item) => (
    ...
    >
      ...
      <Text style={[s.menuChevron, item.disabled && { color: C.muted }]}>›</Text>
    </TouchableOpacity>
  );
  ...
  const menuSections = [
      ...
      items: [
        {
          icon: '👤',
          label: 'Profile',
          sub: 'Raymond · rayray579@gmail.com',
    ...
    },
    {
      title: 'Data & Storage',
      items: [
        { icon: '🐾', label: 'Pets', sub: countsLoading ? 'Loading...' : `${safePets.length}`, onPress: handleDataStoragePress },
        { icon: '📋', label: 'Health Records', sub: countsLoading ? 'Loading...' : `${healthRecordCount}`, onPress: handleDataStoragePress },
        { icon: '⏰', label: 'Care Reminders', sub: countsLoading ? 'Loading...' : `${reminderCount}`, onPress: handleDataStoragePress },
        { icon: '🖼️', label: 'Memories', sub: countsLoading ? 'Loading...' : `${storageCounts.memories ?? 0}`, onPress: handleDataStoragePress },
        { icon: '💬', label: 'Community Posts', sub: countsLoading ? 'Loading...' : `${storageCounts.communityPosts ?? 0}`, onPress: handleDataStoragePress },
        { icon: '🍲', label: 'Recipes', sub: countsLoading ? 'Loading...' : `${storageCounts.recipes ?? 0}`, onPress: handleDataStoragePress },
        { icon: '🗨️', label: 'Comments', sub: countsLoading ? 'Loading...' : `${storageCounts.comments ?? 0}`, onPress: handleDataStoragePress },
        { icon: '🚨', label: 'Lost Pet Alerts', sub: countsLoading ? 'Loading...' : `${storageCounts.lostPetAlerts ?? 0}`, onPress: handleDataStoragePress },
    ...
    },
    {
      title: 'App Info',
      items: [
        { icon: '🧩', label: 'App Name', sub: 'PetSync+', onPress: handleAppInfoPress },
        { icon: '🔢', label: 'Version', sub: 'v1.0.0', onPress: handleAppInfoPress },
        { icon: '☁️', label: 'Supabase', sub: supabaseConnectedStatus, onPress: handleAppInfoPress },
      ],
    },
    {
      title: 'Support',
      ...
      <SafeAreaView style={s.screen} edges={['top']}>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <TouchableOpacity
            ...
            onPress={() => Alert.alert('Logout', 'Account logout is coming soon.')}
          >
            <Text style={s.signOutText}>Logout (Coming soon)</Text>
          </TouchableOpacity>
        ...
        <Modal visible={showAccountModal} transparent animationType="fade" onRequestClose={() => setShowAccountModal(false)}>
          <View style={s.modalOverlay}>
            <View style={s.customActionModal}>
              <Text style={s.customActionModalTitle}>Profile</Text>
              <Text style={{ color: C.text, marginBottom: 10 }}>Raymond · rayray579@gmail.com</Text>
              <Text style={{ color: C.text, marginBottom: 18 }}>
                Profile editing is coming soon. Pet management is available from the pet list below.
              </Text>
              <View style={s.customActionModalButtons}>
                <TouchableOpacity
                  style={[s.customActionSaveBtn, { flex: 1 }]}
