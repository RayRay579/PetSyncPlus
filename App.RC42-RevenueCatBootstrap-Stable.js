...
import { initializeRevenueCatForUser, restoreRevenueCatPurchases as restoreRevenueCatPurchasesNative } from './revenueCat';
...
function SettingsScreen({ navigation, route }) {
  ...
  const currentPlanLabel = revenueCatPremiumActive ? 'Premium' : 'Free';
          ...
          <Modal visible={showSubscriptionModal} transparent animationType="fade" onRequestClose={() => setShowSubscriptionModal(false)}>
            <View style={s.modalOverlay}>
              <View style={[s.customActionModal, s.subscriptionModal]}>
                ...
                <View style={s.customActionModalButtons}>
                  <TouchableOpacity style={[s.customActionCancelBtn, s.featureLockedPrimaryBtn]} disabled activeOpacity={1}>
                    <Text style={s.featureLockedPrimaryText}>Start Monthly</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[s.customActionCancelBtn, s.featureLockedPrimaryBtn]} disabled activeOpacity={1}>
                    <Text style={s.featureLockedPrimaryText}>Start Yearly</Text>
                  </TouchableOpacity>
                </View>
                <View style={[s.customActionModalButtons, { marginTop: 10 }]}>
                  <TouchableOpacity style={s.customActionCancelBtn} onPress={handleRestorePurchases}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const syncRevenueCat = async () => {
      ...
      try {
        const result = await initializeRevenueCatForUser(authUser.id);
        if (!isActive) return;
        applyRevenueCatState(result);
      } catch (error) {
  ...
  }, [applyRevenueCatState, authReady, authUser?.id]);
