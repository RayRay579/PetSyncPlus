...
const stageSpecs = [
  { stage: 22, name: 'buildQuickActionsForSpecies', out: 'src/models/petQuickActions.js', message: 'Stage 22: Extract pet quick action helper' },
  { stage: 23, name: 'buildStarterReminders', out: 'src/models/starterReminders.js', message: 'Stage 23: Extract starter reminder helper' },
  { stage: 24, name: 'buildStarterHealthRecords', out: 'src/models/starterHealthRecords.js', message: 'Stage 24: Extract starter health record helper' },
  { stage: 25, name: 'calculateAgeLabelFromBirthday', out: 'src/models/petAge.js', message: 'Stage 25: Extract pet age helper' },
  { stage: 26, name: 'buildPetEditDraft', out: 'src/models/petEditDraft.js', message: 'Stage 26: Extract pet edit draft helper' },
  { stage: 27, name: 'getStreakDaysFromEvents', out: 'src/models/streakDays.js', message: 'Stage 27: Extract streak day helper' },
  { stage: 28, name: 'getStreakEventsForPet', out: 'src/models/streakEvents.js', message: 'Stage 28: Extract streak event helper' },
  { stage: 29, name: 'getStreakSummaryForPet', out: 'src/models/streakSummary.js', message: 'Stage 29: Extract streak summary helper' },
  { stage: 30, name: 'getTrendEventsForPet', out: 'src/models/trendEvents.js', message: 'Stage 30: Extract trend event helper' },
  { stage: 31, name: 'getTrendActionBreakdownForPet', out: 'src/models/trendActionBreakdown.js', message: 'Stage 31: Extract trend action breakdown helper' },
];
...
for (const spec of stageSpecs) {
  ...
  try {
    ...
    // Replace the declaration first while the AST offsets are still valid.
    let next = original.slice(0, decl.start) + wrapper + original.slice(decl.end);
    // Add the import only after the replacement, preventing stale-index splices.
...
console.log('\n============================================================');
console.log('STAGES 22-31 COMPLETE, SKIPPED, OR STOPPED SAFELY');
