...
const registerForPushNotificationsAsync = async () => {
  try {
    ...
    if (!projectId) {
      console.log('Push notifications skipped: projectId not available in Expo Go');
      return null;
    }
...
function DashboardScreen({ navigation }) {
  ...
  const [medicationNextDoseDate, setMedicationNextDoseDate] = useState('');
  const [medicationNextDoseTime, setMedicationNextDoseTime] = useState('');
  const [recentActivityY, setRecentActivityY] = useState(0);
  ...
  }, [safePets, setPetScores]);
  const pet = safePets.find(p => p.id === selectedPetId) || safePets[0];
  useEffect(() => {
    if (safePets.length > 0 && !safePets.some((item) => item.id === selectedPetId)) {
      setSelectedPetId(safePets[0].id);
    }
  }, [safePets, selectedPetId]);

  const getDashboardPetAgeLabel = (petItem) => {
    if (!petItem) return 'Age unknown';
    if (petItem.age) return String(petItem.age);

    const birthdayValue = petItem.birthday || petItem.birthDate || petItem.dateOfBirth || petItem.dob;
    if (birthdayValue) {
      ...
      if (!Number.isNaN(birthday.getTime())) {
        ...
        const months = Math.max(0, (today.getFullYear() - birthday.getFullYear()) * 12 + (today.getMonth() - birthday.getMonth()));
        if (months > 0) {
  ...
  const parseDashboardRecordTimestamp = (record) => {
    if (!record) return 0;
    ...
    const dateText = String(record.date || record.record_date || '').trim();
    if (!dateText) return 0;
    const parsed = new Date(dateText);
    return Number.isNaN(parsed.getTime()) ? 0 : parsed.getTime();
  };

  const formatDashboardWeight = (record) => {
    if (!record) return '';

    const rawValue = record.value ?? record.details?.weightValue ?? record.details?.value ?? record.details?.weight ?? '';
    if (rawValue === '' || rawValue == null) return '';

    const weightText = String(rawValue).trim();
    if (!weightText) return '';

    if (weightText.toLowerCase().includes('lb') || weightText.toLowerCase().includes('kg')) {
      return weightText;
    }

    const unit = String(record.unit || record.details?.unit || 'lbs').trim();
    return `${weightText} ${unit || 'lbs'}`;
  };
  ...
  const playReminderPetSound = useCallback(async (petId) => {
    const reminderPet = safePets.find((item) => item.id === petId);
  ...
  const handleSelectPet = (petId) => {
    ...
    const selectedPet = safePets.find(p => p.id === petId);
  ...
  const getReminderCategory = (reminder) => {
    const text = `${reminder?.title || ''} ${reminder?.icon || ''} ${reminder?.source || ''}`.toLowerCase();

    if (text.includes('urgent') || text.includes('emergency') || text.includes('critical') || text.includes('overdue') || text.includes('🚨')) {
      return 'urgent';
    }
    if (text.includes('med') || text.includes('medication') || text.includes('pill') || text.includes('dose') || text.includes('💊')) {
      return 'medication';
    }
    if (
      text.includes('vac') ||
  ...
  const dashboardTrendTypes = new Set(['meal', 'walk', 'weight', 'medication', 'grooming']);
  const selectedCalendarDateKey = formatLocalDateKey(selectedCalendarDate);
  const calendarReminders = safeCareReminders.filter(reminder => {
    return (
  ...
  const remindersThisWeek = safeCareReminders.filter((reminder) => {
  ...
  const calendarReminderCounts = calendarDays.reduce((acc, date) => {
    ...
    acc[key] = safeCareReminders.filter((reminder) => reminder.petId === selectedPetId && reminder.date === key).length;
    return acc;
  }, {});
  ...
  const activityTrendCounts = activityTrendDays.map((date) => {
    const key = formatLocalDateKey(date);
    return safeActivityLogs.filter((log) => {
      const normalizedType = String(log?.type || '').toLowerCase();
      if (!dashboardTrendTypes.has(normalizedType)) return false;
      if (log.petId !== selectedPetId) return false;
      if (log.dateKey === date.toDateString() || log.dateKey === key) return true;
      if (typeof log.createdAt === 'number') {
        return formatLocalDateKey(new Date(log.createdAt)) === key;
      }
      return false;
    }).length;
  });
  const activityTrendMax = Math.max(1, ...activityTrendCounts);
  const hasTrendActivity = activityTrendCounts.some((count) => count > 0);
  ...
  const alertDueReminder = useCallback(async (reminder) => {
    ...
    const reminderPet = safePets.find((item) => item.id === reminder.petId);
  ...
  useEffect(() => {
    const checkDueRemindersNow = () => {
      ...
      const dueReminder = careReminders.find((reminder) => {
        ...
        if (reminder.petId && !safePets.some((petItem) => petItem.id === reminder.petId)) return false;
  ...
  }, [alertDueReminder, getReminderScheduledDate, safeCareReminders, safePets, setReminderAlerted]);
  ...
  const toggleTask = (taskId) => {
    ...
    if (!task.done) {
      ...
      const matchingTaskLog = safeActivityLogs.find(log => log.source === 'task' && log.taskId === task.id && log.petId === task.petId);
      ...
      if (matchingTaskLog) {
        ...
        if (scoreDelta > 0) {
          setPetScores(prev => ({
            ...prev,
            [task.petId]: Math.max(0, Math.min(100, (prev[task.petId] ?? safePets.find(p => p.id === task.petId)?.score ?? 0) - scoreDelta)),
          }));
        }
      }
    }
  ...
  const addActivityLog = (type, title, icon, extra = {}) => {
    const now = new Date();
    const targetPetId = extra.petId ?? selectedPetId;
    const newLog = {
      id: Date.now().toString(),
      petId: targetPetId,
      type,
    ...
    setPetScores(prev => {
      const current = prev[targetPetId] ?? safePets.find(p => p.id === targetPetId)?.score ?? 80;
      return {
        ...prev,
  ...
  const deleteActivityLog = (logId) => {
    const logToDelete = safeActivityLogs.find(log => log.id === logId);
    ...
    if (logToDelete) {
      ...
      setPetScores(prev => {
        const current = prev[logToDelete.petId] ?? safePets.find(p => p.id === logToDelete.petId)?.score ?? 80;
        return {
          ...prev,
  ...
  const currentPetId = pet?.id || selectedPetId || '';
  const selectedPetHealthRecords = safeHealthRecords.filter((record) => record.petId === currentPetId);
  const latestWeightRecord = selectedPetHealthRecords
    .filter((record) => record.type === 'weight')
    .slice()
    .sort((a, b) => parseDashboardRecordTimestamp(b) - parseDashboardRecordTimestamp(a))[0] || null;
  const fallbackPetWeight = pet?.weight
    ? (String(pet.weight).toLowerCase().includes('lb') ? String(pet.weight) : `${pet.weight} lbs`)
    : 'Weight not set';
  const latestWeightLabel = formatDashboardWeight(latestWeightRecord) || fallbackPetWeight;
  if (!pet) {
    return (
  ...
  const recentActivity = safeActivityLogs
      ...
      if (item.label === 'Medication') {
        return {
          ...
          action: () => {
            ...
            setMedicationDose('');
            setMedicationNotes('');
            setMedicationNextDoseDate('');
            setMedicationNextDoseTime('');
  ...
  const saveQuickMedicationRecord = () => {
    ...
    const trimmedNotes = medicationNotes.trim();
    const trimmedNextDoseDate = medicationNextDoseDate.trim();
    const trimmedNextDoseTime = medicationNextDoseTime.trim();
    if (!trimmedName) {
    ...
    const nextRecord = {
      ...
      petId: selectedPetId,
      type: 'medication',
      title: `Medication: ${trimmedName}`,
      date: todayKey,
      nextDue: trimmedNextDoseDate || '',
      notes: trimmedNotes || trimmedDose,
      details: {
        medicationName: trimmedName,
        dosage: trimmedDose,
        frequency: 'As directed',
        startDate: todayKey,
        medicationNotes: trimmedNotes || trimmedDose,
        nextDoseDate: trimmedNextDoseDate || '',
        nextDoseTime: trimmedNextDoseTime || '',
        source: 'dashboard_quick_action',
      },
      icon: '💊',
      status: 'current',
    ...
    if (trimmedNextDoseDate) {
      const reminderPayload = {
        id: Date.now().toString(),
        petId: selectedPetId,
        title: `Medication: ${trimmedName}`,
        icon: '💊',
        date: trimmedNextDoseDate,
        time: trimmedNextDoseTime || '',
        completed: false,
        source: 'medication',
        sourceRecordId: nextRecord.id,
      };
      setCareReminders((prev) => [reminderPayload, ...prev]);
      saveCareReminderToSupabase(reminderPayload);
    }
    addActivityLog('medication', 'Medication given', '💊');
    setMedicationName('');
    setMedicationDose('');
    setMedicationNotes('');
    setMedicationNextDoseDate('');
  ...
  return (
    ...
    <SafeAreaView style={[s.screen, { backgroundColor: 'transparent' }]} edges={['top']}>
      ...
      >
          ...
          <View style={s.dashHeader}>
            <View style={{ flex: 1, paddingRight: 60 }}>
              ...
              <Text style={s.subGreeting} numberOfLines={1}>
                {formatDate(new Date())} · {safePets.length} pets
              </Text>
            </View>
        ...
        <Card style={[s.healthScoreCard, { backgroundColor: 'rgba(248,250,255,0.94)', borderColor: 'rgba(126,87,194,0.14)' }]}>
            <View style={s.dashboardHeroTop}>
              ...
              <View style={s.dashboardHeroInfo}>
                ...
                <Text style={s.dashboardPetAge}>
                  {getDashboardPetAgeLabel(pet)} • {latestWeightLabel}
                </Text>
            ...
            <View style={s.dashboardHeroMetricsRow}>
              ...
              <View style={s.dashboardMiniCard}>
                ...
                {nextReminder ? (
                  ...
                  <Text style={s.dashboardMiniCardSub}>No upcoming reminders</Text>
                )}
              </View>
            </View>
          </Card>
        ...
        <TouchableOpacity activeOpacity={0.92} onPress={() => {
          if (dashboardScrollRef.current?.scrollTo) {
            dashboardScrollRef.current.scrollTo({
              y: Math.max(0, recentActivityY - 12),
              animated: true,
            });
          }
        }}>
          <Card
              ...
              s.dashboardTrendCard,
            {
              ...
              paddingBottom: hasTrendActivity ? 12 : 8,
              minHeight: hasTrendActivity ? undefined : 182,
            },
            ]}
          >
          ...
          {hasTrendActivity ? (
            <View style={s.dashboardTrendChart}>
              {activityTrendCounts.map((count, index) => {
                ...
                const dayLabel = ['S', 'M', 'T', 'W', 'T', 'F', 'S'][activityTrendDays[index].getDay()];
                return (
                  <View key={`trend-bar-${index}`} style={s.dashboardTrendBarWrap}>
                    <View style={[s.dashboardTrendBar, { height, backgroundColor: '#E8ECF7' }]}>
                      <View style={[s.dashboardTrendBarFill, { height, backgroundColor: '#6B4BFF' }]} />
                    </View>
                    <View style={[s.dashboardTrendDot, { backgroundColor: count > 0 ? '#49C2A8' : '#D7DDEA' }]} />
            ...
            <View style={[s.dashboardTrendEmpty, { minHeight: 76, height: 76, marginTop: 2 }]}>
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.dashboardAISub}>Ask about symptoms, medications, food safety, or care concerns.</Text>
          </View>
        ...
        <View style={[s.section, { marginTop: 8 }]}>
          ...
          >
            {calendarDays.map((date) => {
              ...
              const reminderDotsForDate = (safeCareReminders || [])
                .filter((reminder) => reminder.petId === selectedPetId && reminder.date === formatLocalDateKey(date))
                .map(getReminderCategory)
                .slice(0, 3);
              const dotColorMap = {
                medication: '#6B4BFF',
                appointment: '#FF9F43',
                ...
                >
                  {selected ? (
                    <Animated.View style={{ transform: [{ scale: calendarChipBounce }] }}>
                      ...
                      {reminderDotsForDate.length > 0 ? (
                        <View style={s.dashboardCalendarDotWrap}>
                          <View style={s.dashboardCalendarDotRow}>
                            {reminderDotsForDate.map((dotType, dotIndex) => (
                              <View
                                key={`${formatLocalDateKey(date)}-${dotType}-${dotIndex}`}
                                style={[
                                  s.dashboardCalendarDot,
                                  { backgroundColor: dotColorMap[dotType] || C.teal },
                                ]}
                              />
                            ))}
                          </View>
          ...
          {calendarReminders.length === 0 ? (
            <Card style={[{ backgroundColor: 'rgba(248,250,255,0.90)', borderColor: 'rgba(126,87,194,0.14)' }]}>
              <Text style={{ color: '#7C8798', textAlign: 'center', padding: 16 }}>
            ...
            calendarReminders.map(reminder => (
              ...
              >
                <Card style={[s.reminderCard, { backgroundColor: 'rgba(248,250,255,0.90)', borderColor: 'rgba(126,87,194,0.14)' }, reminder.completed && s.reminderCardDone]}>
        ...
        <View
          ...
          onLayout={(event) => setRecentActivityY(event.nativeEvent.layout.y)}
        >
          <Text style={[s.sectionTitle, { color: '#152033' }]}>Recent Activity</Text>
          {recentActivity.length === 0 ? (
            <Card style={[s.recentActivityEmptyCard, { backgroundColor: 'rgba(248,250,255,0.90)', borderColor: 'rgba(126,87,194,0.14)' }]}>
            ...
            recentActivity.map(log => (
              ...
              >
                <Card style={[s.recentActivityCard, { backgroundColor: 'rgba(248,250,255,0.90)', borderColor: 'rgba(126,87,194,0.14)' }]}>
        ...
        <View style={s.modalOverlay}>
          <View style={s.customActionModal}>
            ...
            <View style={s.customActionModalButtons}>
              ...
              <TouchableOpacity style={s.customActionSaveBtn} onPress={handleSaveWeight}>
      ...
      <Modal visible={showMedicationModal} transparent animationType="fade" onRequestClose={() => {
        ...
        <View style={s.modalOverlay}>
          <View style={s.customActionModal}>
            ...
            <TextInput
              style={s.customActionInput}
              value={medicationNextDoseDate}
              onChangeText={setMedicationNextDoseDate}
              placeholder="Next dose date (YYYY-MM-DD)"
              placeholderTextColor={C.muted}
            />
            <TextInput
              style={s.customActionInput}
              value={medicationNextDoseTime}
              onChangeText={setMedicationNextDoseTime}
              placeholder="Next dose time (optional)"
              placeholderTextColor={C.muted}
            />
            <View style={s.customActionModalButtons}>
              <TouchableOpacity style={s.customActionCancelBtn} onPress={() => {
